...
def find_probability(class_probability,curr_dict,other_dict,vocabulary,num_words):
    ...
    prob_tmp = [ math.log((each_value+1)/(num_words + vocabulary)) for each_value in value_tmp]
...
for every_fl in dev_data:
    ...
    ham_probability = find_probability(prob_ham,dict_ham,dict_spam,vocabulary,total_hamWords)
    spam_probability = find_probability(prob_spam,dict_spam,dict_ham,vocabulary,total_spamWords)
